...
impl Vault {
    pub fn initialize(config: VaultConfig) -> VaultResult<Self> {
        std::fs::create_dir_all(&config.data_dir)
            .map_err(|e| VaultError::Storage(format!("Dir: {}", e)))?;

        let mid_path = config.data_dir.join("machine_id");
        let machine_id = if mid_path.exists() {
            let d = std::fs::read(&mid_path)
                .map_err(|e| VaultError::Storage(format!("Read MID: {}", e)))?;
            if d.len() != 32 {
                return Err(VaultError::Integrity("MID corrupt".into()));
            }
            let mut id = [0u8; 32];
            ...
            std::fs::write(&mid_path, id)
                .map_err(|e| VaultError::Storage(format!("Write MID: {}", e)))?;
            id
        };
        ...
        let integrity_hmac_key = if hmac_path.exists() {
            let d = std::fs::read(&hmac_path)
                .map_err(|e| VaultError::Storage(format!("Read HMAC: {}", e)))?;
            if d.len() != 32 {
                return Err(VaultError::Integrity("HMAC corrupt".into()));
            }
            let mut k = [0u8; 32];
            k.copy_from_slice(&d);
            k
            ...
            std::fs::write(&hmac_path, k)
                .map_err(|e| VaultError::Storage(format!("Write HMAC: {}", e)))?;
            k
        };
        ...
        let al = Arc::new(AuditLogger::new(
            config
                ...
                .join("audit.db")
                .to_str()
                .ok_or_else(|| VaultError::Config("Invalid audit path".into()))?,
            machine_id,
            Some(1000),
            Some(1_073_741_824),
        )?);
        ...
        match provider {
            "tpm" => {
                let tpm = self
                    .tpm_manager
                    .as_ref()
                    .ok_or_else(|| VaultError::Tpm("TPM not available".into()))?;
                let tsp = self.config.data_dir.join("tpm_state.db");
                let tss = tsp
                    .to_str()
                    .ok_or_else(|| VaultError::Config("Invalid path".into()))?;

                let mk = if Path::new(tss).exists() {
                    let state = tpm.load_state(tss, &self.integrity_hmac_key)?;
                    tpm.unseal_with_pcr_validation(&state, &self.integrity_hmac_key)?
                } else {
    ...
    pub fn check_auto_lock(&mut self) -> VaultResult<()> {
        ...
        if let Some(timeout) = self.config.auto_lock_timeout_seconds {
            if let Some(last) = self.last_activity {
                if last.elapsed().as_secs() > timeout {
